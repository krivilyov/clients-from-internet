			...
			<div class="main-row">
				<?php if(!empty($cards)): ?>
					<?php
						...
						function tplSubCards ($isAuth, $card, $i) {
							...
							if(count($card['children']) > 0){
								$tplSubCards .= renderSubCards($isAuth, $card['children']);
							}
						...
						function renderSubCards ($isAuth, $cards, $i = 1) {
							$subCardsHTML = '';

							foreach($cards as $card){
								$subCardsHTML .= tplSubCards($isAuth, $card, $i);
							}
					...
					<?php
						$subCards = renderSubCards($isAuth, $cards, $i = 0);
						echo $subCards;
					?>
				<?php endif ?>
			</div>
				...
				<?php if(isset($allCards)): ?>
					<select name="parentId">
						...
						<?php foreach($allCards as $card): ?>
							<option value="<?php echo $card['id'] ?>" <?php if(!$isAuth): ?>disabled="disabled"<?php endif ?>><?php echo $card['title'] ?></option>
						<?php endforeach; ?>
					</select>
				<?php endif ?>
